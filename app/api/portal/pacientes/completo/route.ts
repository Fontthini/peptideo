...
// Espelha /api/admin/pacientes/completo — mesmo wizard consolidado (quem
// indicou, receita, dados, produto/pedido com desconto, comprovante,
// documentos, cashback%), só que autenticado por token de membro (gerente)
// em vez da chave de admin.
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    pedidoCriado = p;
    mem_registrarLog(ator, 'Cadastrou paciente manualmente (completo, portal)', `${nomeCompleto} (indicado por ${medico.nome}) — ${produto.nome} — R$ ${valorPago.toFixed(2)}`);
    ...
    try { const { sbSaveIndicacao } = await import('@/lib/supabase-sync'); await sbSaveIndicacao(indicacaoFinal); } catch (e) { console.error('[PORTAL-PACIENTE-COMPLETO] sync error:', e); }

    return NextResponse.json({ indicacao: indicacaoFinal, pedido: p }, { status: 201 });
  } catch (e) {
    console.error('[PORTAL-PACIENTE-COMPLETO] erro, desfazendo o que já foi criado:', e);
    if (pedidoCriado) {
