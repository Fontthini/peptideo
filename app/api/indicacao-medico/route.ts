...
export async function POST(req: NextRequest) {
  try {
    ...
    const { token, nome, sobrenome, whatsapp, email, endereco, crm } = body;

    if (!token || !nome || !whatsapp || !endereco || !crm) {
    ...
    try {
      ...
      console.error('[INDICACAO-MEDICO] Supabase save error:', e);
    }
    ...
    console.error('[INDICACAO-MEDICO]', err);
